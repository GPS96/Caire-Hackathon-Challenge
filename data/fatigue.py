...
facial indicators (blink_rate, avg_blink_duration, yawns_count,
mouth_open_duration, head_nod_count, head_pitch_std) with HRV indicators
(heart_rate, mean_rr, std_rr, optional rmssd).

Inputs are aggregated per analysis window (e.g., 30–90 s). The module returns
a normalized fatigue score in [0, 100] and an alert decision.
"""
...
def facial_score_from_features(features: Dict[str, float]) -> float:
    """Compute facial sub-score in [0,1]. Higher = more drowsy."""
    ...
    blink_rate_score = normalize(blink_rate, 6.0, 30.0)            # ↑ drowsy
    blink_dur_score  = normalize(avg_blink_dur, 0.05, 0.5)         # ↑ drowsy
    yawns_score      = normalize(yawns, 0.0, 3.0)                   # ↑ drowsy
    mouth_score      = _clamp01(mouth_open)                         # 0..1 fraction
    nods_score       = normalize(nods, 0.0, 4.0)                    # ↑ drowsy
    pitch_score      = normalize(pitch_std, 0.1, 6.0)               # ↑ drowsy
...
def hrv_score_from_features(hrv_features: Dict[str, float]) -> float:
    """Compute HRV-derived sub-score in [0,1]. Lower HRV → higher score."""
    ...
    # Prefer RMSSD if available
    if rmssd >= 0:
        hrv_norm = normalize(rmssd, 10.0, 80.0, invert=True)       # lower rmssd → worse
    else:
        hrv_norm = normalize(std_rr, 0.02, 0.18, invert=True)      # lower std_rr → worse

    # These two give additional context (kept broad here)
...
def compute_subscores_and_score(
    ...
    # Facial features (to 0..1 then 0..100)
    ...
    # If baseline present, use deltas (more personalized)
    if baseline and _safe_float(baseline.get("heart_rate", 0.0)) > 0:
        ...
        # Note: narrower mapping in this personalized branch is intentional
    else:
        s_hr      = normalize(heart_rate, 55.0, 90.0)   # tighter band for “typical driving”
...
def combined_fatigue_score(
    ...
    """Combine facial and HRV sub-scores into a final score in [0,100]."""
