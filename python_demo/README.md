...
$env:IMAGES_DIR=".\images"; $env:FPS="30"; $env:DURATION_SEC="30"; $env:RES_WIDTH="640"; $env:RES_HEIGHT="480"; uv run record.py
```
...
```powershell
$env:API_KEY="YOUR_API_KEY"; $env:BACKEND_WS_BASE="ws://localhost:8003/ws/"; $env:IMAGES_DIR=".\images"; $env:FPS="30"; uv run client.py
```

---

## Live end‑to‑end pipeline (no disk)

Use `live_pipeline.py` to capture webcam, stream frames to the backend, receive **rPPG** live, convert to **clean PPG**, and append rows to `live_ppg.csv`.
...
2. Edit `.env` and fill in your `API_KEY`, `BACKEND_WS_BASE`, and other configuration values.
...
- `live_ppg.csv` with columns: `time_s`, `rppg`, `ppg`
- Live preview window showing camera feed (if `SHOW_PREVIEW=1` in `.env`)

**Stop:**

Press `Ctrl+C` to terminate. The script will flush and close the CSV file gracefully.

**Configuration (`.env` file):**

- `API_KEY` — your API key
- `BACKEND_WS_BASE` — e.g. `ws://localhost:8003/ws/`
- `FPS` — target send rate (default: `30`)
- `CAMERA_INDEX` — camera device index (default: `0`)
- `SHOW_PREVIEW` — show live camera feed window (default: `1`)
- `JPEG_QUALITY` — JPEG quality 1..100 (default: `75`)
- `RPPG_BUFFER_SEC` — seconds of rPPG history for the cleaner (default: `20`)
- `LIVE_CSV` — output CSV file path (default: `live_ppg.csv`)

**Notes:**

- The server response must contain an rPPG array and, ideally, an `fs` sampling rate. If your schema differs, adapt `extract_rppg()` in `live_pipeline.py` (it already checks common shapes like `advanced.rppg`, `data.rppg`).
- PowerShell does not support Bash-style `KEY=VALUE` prefixes; always set env with `$env:KEY="value"` or use `.env` file.

---

## Live visualization of CSV data

Use `plot_live_csv.py` to visualize the rPPG and PPG signals in real-time as they're being written to the CSV file. This runs separately from the main pipeline for better performance.

**Run (in a separate terminal):**

```powershell
cd python_demo
uv run plot_live_csv.py
```

Or specify a custom CSV file:

```powershell
uv run plot_live_csv.py my_custom_file.csv
```

**Features:**

- Real-time scrolling plot showing last 10 seconds of data
- Two subplots: rPPG (raw) in red, PPG (cleaned) in blue
- Updates every 100ms for smooth animation
- Automatically starts when CSV file is created
- Shows sample count and elapsed time in title

**Usage pattern:**

1. Terminal 1: Run `uv run live_pipeline.py` (captures webcam and writes CSV)
2. Terminal 2: Run `uv run plot_live_csv.py` (visualizes the CSV in real-time)

This separation provides much smoother visualization without impacting the capture/processing performance.
